...
| Setting | Description |
| :--- | :--- |
| text_font_priority | A list of font types. If the first one is installed, the program will use it, otherwise it'll check the others in the written order |
...
| independent_windows | If true, each instance of this editor will spawn its own process. Slightly slower at startup, but recommended if you often open big files. If false (experimental) then opening multiple small files will be faster |
